    ...
    try:
        while True:
            if arduino.in_waiting > 0:
                ...
                if distanza.isdigit():
                    distance_label.config(text=f"Container Level: {distanza} %")         
                    temperature_label.config(text=f"Temperature: {temperatura} °")
...
distance_label = tk.Label(root, text="Container Level: -- %", fg="yellow3", font=("Rockwell", 16))
...
temperature_label = tk.Label(root, text="Temperature: -- °", fg="yellow3", font=("Rockwell", 16))
